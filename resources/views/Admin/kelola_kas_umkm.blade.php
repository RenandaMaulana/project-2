...
<body>
        ...
        <div class="container-fluid page-body-wrapper">
            ...
            <nav class="navbar col-lg-12 col-12 px-0 py-0 py-lg-4 d-flex flex-row">
                <div class="navbar-menu-wrapper d-flex align-items-center justify-content-end">
                    ...
                    <div class="navbar-brand-wrapper">
                        <a class="navbar-brand brand-logo" href="spica/index.html"></a>
                        <a class="navbar-brand brand-logo-mini" href="spica/index.html"></a>
                    </div>
                    <h4 class="font-weight-bold mb-0 d-none d-md-block mt-1">
                        Kelola Kas UMKM Alun-Alun Subang
                    </h4>
            ...
            <div class="main-panel">
                <div class="content-wrapper">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <h4>Kelola kas UMKM</h4>
                            <button type="button" class="btn btn-info btn-icon-text">
                                <a href={{ route('tambahkas_umkm') }} type="button">
                            ...
                            <div class="table-responsive">
                                <table id="mytable" class="table table-bordred table-striped">
                                    ...
                                    @foreach ($kas as $key => $Kas)
                                        <!--$Kas pertama terdapat pada models, dan user kedua variabel-->
                                        <tbody>
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>{{ $Kas->name }}</td>
                                                <td>{{ $Kas->email }}</td>
                                                <td><a href="{{ asset($Kas->berkas) }}">{{ $Kas->berkas }}</td>
                                                <td>
                                                    @if ($Kas->status == 'diterima')
                                                ...
                                                <td>
                                                    <a href="/kelola-kas-umkm/detail/{{ $Kas->id }}"
                                                        type="button" class="btn btn-info"><i
                                                            class="mdi mdi-file-find"></i></a>
                                                </td>
                                                <td>
                                                    <a onclick="handleTerima('{{ $Kas->id }}', 'diterima')"
                                                        class="btn btn-success" type="button">
                                                        <i class="mdi mdi-check"></i>
                                                    </a>
                                                </td>
                                                <td>
                                                    <a onclick="handleDitolak('{{ $Kas->id }}', 'ditolak')"
    ...
    <script>
        function handleTerima(id, status) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('kas.updatestatus-kas', ['id' => ':id', 'status' => 'diterima']) }}"
                            .replace(':id', id),
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
    ...
    <script>
        function handleDitolak(id, status) {
            Swal.fire({
                title: "Apakah Anda Yakin ?",
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('kas.updatestatus-kas', ['id' => ':id', 'status' => 'ditolak']) }}"
                            .replace(':id', id),
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
